...
- **자동 인식** — `<sts2>/mods/` 하위를 **재귀적으로** 스캔해 세 종류 모드를 자동 감지 (예: `mods/캐릭터/`, `mods/아트워크/`, `mods/유틸/` 처럼 카테고리 폴더로 정리해도 인식. `.pck` 파일명이 다르면 어떤 깊이든 OK — 같은 이름의 `.pck` 가 둘 이상 있으면 첫 번째만 유지하고 경고 로그). STS2 본체도 `mods/` 를 재귀적으로 스캔하므로 DLL+manifest+pck 가 묶인 모드도 깊이 상관없이 작동해요 — **단, 옮긴 뒤 원래 `mods/<modName>/` 폴더는 반드시 삭제**해주세요. 안 그러면 두 위치 모두 발견돼서 `DUPLICATE_ID` 에러가 납니다:
  ...
  - **이벤트 배경 모드** — Ancient NPC 초상화 (`images/ancients/`) 나 이벤트 씬 배경 (`images/events/`) 만 리텍스처하는 모드 (예: AncientRetexture). "다른 모드" 탭에서 단일 on/off 체크박스로 노출
...
- **메뉴 드래그 이동** — 접힌 헤더 왼쪽의 **⋮⋮ 그립**을 드래그하면 오버레이 전체(드롭다운·미리보기·패널)를 원하는 위치로 옮길 수 있음. 같은 코너를 쓰는 다른 모드의 메뉴바와 겹칠 때 사용. **그립 더블클릭**으로 원위치. 위치는 다음 실행에도 유지되며, 화면 아래쪽 절반으로 옮기면 패널이 **위로 펼쳐져** 화면 밖으로 나가지 않음.
- **오버레이 위치 설정** — 캐릭터 선택 화면 오버레이 위치를 좌상단/우상단 중 선택. 기본값은 **우상단** (게임의 멀티플레이 로비 패널이나 좌상단 기본 UI 와의 충돌 회피용). **RitsuLib** 또는 [Nexus ModConfig](https://www.nexusmods.com/slaythespire2/mods/27) 설치 시 인게임 dropdown 으로 좌상단(v0.8 레이아웃)으로 즉시 복귀 가능 — 재시작 불필요. 둘 다 없어도 무관: 그립 드래그는 설정 프레임워크 없이 동작.
...
- **우상단** (기본값, RitsuLib / [ModConfig](https://www.nexusmods.com/slaythespire2/mods/27) 로 좌상단 전환 가능, 또는 그냥 드래그): `Skin [<캐릭터>]:` 드롭다운. 캐릭터 클릭 → variant 선택.
- **`스킨 매니저` 헤더 왼쪽 ⋮⋮ 그립**: 드래그로 메뉴 전체 이동, 더블클릭으로 위치 초기화.
...
Save 할 때마다 Sts2SkinManager 가 `<sts2>/mods/Sts2SkinManager/modpack_preset.preset` 에도 동일한 내용을 자동으로 mirror 합니다. 친구에게 본인 모드팩을 통째로 보내려면:

1. 게임 내에서 원하는 조합으로 설정하고 `Save` 클릭.
2. `<sts2>/mods/` 폴더를 통째로 zip (또는 공유하고 싶은 스킨/카드 모드들 + `Sts2SkinManager` 폴더).
3. zip 전송. 받는 친구는 자기 `<sts2>/mods/` 에 압축 해제. 첫 실행 시 Sts2SkinManager 가 `modpack_preset.preset` 으로부터 `skin_choices.json` 을 자동 seed — 드롭다운 선택, 카드 스킨 순서, 토글까지 그대로 적용됩니다.

받는 측 PC 에 preset 이 가리키는 모드가 없으면 그 항목만 조용히 `default` (기본 게임 아트) 로 떨어지고 나머지는 정상 적용.

> **Nexus 배포자 주의:** 릴리즈 zip 에 `modpack_preset.preset` 을 **포함하면 안 됨**. 포함될 경우 사용자가 업데이트를 설치하는 순간 본인이 가진 preset 이 덮어쓰여집니다. 표준 릴리즈 zip (DLL + manifest + README + LICENSE) 은 이미 preset 을 포함하지 않습니다.
>
> v0.12.1 이하에서는 파일명이 `modpack_preset.json` 이었습니다. STS2 본체 ModManager 가 `mods/` 트리의 모든 `.json` 을 manifest 후보로 deserialize 시도해 매부팅마다 "missing the 'id' field" 에러가 1줄씩 기록되던 문제를 v0.12.2 에서 확장자 변경으로 해결했습니다. 기존 preset 은 첫 부팅 시 자동 rename 됩니다.
